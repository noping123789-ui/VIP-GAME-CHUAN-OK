...
export class VideoRecorderService {
  ...
  public drawBroadcastHUD(
    ...
    // OVERLAY DANH SÁCH TAY ĐUA (Từ giây thứ 3 đến giây thứ 6)
    if (elapsedSecs >= 3.0 && elapsedSecs <= 6.0) {
      ctx.save();
      // Nền bán trong suốt sang trọng chuẩn truyền hình
      ctx.fillStyle = 'rgba(5, 7, 14, 0.96)';
      ctx.fillRect(0, 0, 1080, 1920);

      // Tiêu đề Starting Grid
      ctx.fillStyle = '#06b6d4';
      ctx.font = 'bold 36px monospace';
      ctx.textAlign = 'center';
      ctx.fillText('GRAND PRIX STARTING GRID', 540, 150);

      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 44px sans-serif';
      ctx.fillText('DANH SÁCH TAY ĐUA XUẤT PHÁT', 540, 210);

      ctx.fillStyle = '#64748b';
      ctx.font = '24px monospace';
      ctx.fillText(`LUỒNG ĐUA #${instance.id.toString().padStart(2, '0')} • SEED #${seed}`, 540, 265);

      // Đường kẻ trang trí cyan
      ...
      // Vẽ tối đa 10 xe đua
      const sortedCars = [...instance.cars].sort((a, b) => (a.state.rank || 1) - (b.state.rank || 1));
      const startY = 350;
      const rowHeight = 115;

      sortedCars.slice(0, 10).forEach((car, idx) => {
        const y = startY + idx * rowHeight;
        
        // Background card
        ctx.fillStyle = 'rgba(15, 23, 42, 0.85)';
        ctx.fillRect(100, y, 880, 95);
        ctx.strokeStyle = 'rgba(51, 65, 85, 0.5)';
        ctx.lineWidth = 2;
        ctx.strokeRect(100, y, 880, 95);

        // Grid Position Badge
        ctx.fillStyle = 'rgba(6, 182, 212, 0.1)';
        ctx.fillRect(120, y + 15, 65, 65);
        ctx.strokeStyle = 'rgba(6, 182, 212, 0.3)';
        ctx.lineWidth = 2;
        ctx.strokeRect(120, y + 15, 65, 65);

        ctx.fillStyle = '#06b6d4';
        ctx.font = 'bold 34px monospace';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(`${idx + 1}`, 152, y + 48);

        // Team Color vertical bar
        const colorHex = car.state.hexColor !== undefined ? `#${car.state.hexColor.toString(16).padStart(6, '0')}` : '#00f0ff';
        ctx.fillStyle = colorHex;
        ctx.fillRect(205, y + 15, 12, 65);
        ...
        ctx.fillText((car.state.driverName || '').toUpperCase(), 235, y + 18);

        // Car Name
        ctx.fillStyle = '#94a3b8';
        ctx.font = '22px monospace';
        ctx.fillText((car.state.name || '').toUpperCase(), 235, y + 52);

        // Category Badge
        ctx.fillStyle = 'rgba(6, 182, 212, 0.15)';
        ctx.fillRect(840, y + 27, 110, 42);
        ctx.strokeStyle = 'rgba(6, 182, 212, 0.3)';
        ctx.strokeRect(840, y + 27, 110, 42);

        ctx.fillStyle = '#06b6d4';
        ctx.font = 'bold 22px monospace';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(car.state.type === 'hypercar' ? 'HYPER' : 'F1', 895, y + 48);
      });
      ...
      ctx.fillText('CUỘC ĐUA SẮP BẮT ĐẦU • KHỞI ĐỘNG ĐỘNG CƠ', 540, 1860);
