...
export class CarModelManager {
  ...
  private normalizeModel(rawObject: THREE.Object3D, def: CarModelDefinition): THREE.Group {
    ...
    const flGeos: { mesh: THREE.Mesh; pos: THREE.Vector3 }[] = [];
    const frGeos: { mesh: THREE.Mesh; pos: THREE.Vector3 }[] = [];
    const rlGeos: { mesh: THREE.Mesh; pos: THREE.Vector3 }[] = [];
    const rrGeos: { mesh: THREE.Mesh; pos: THREE.Vector3 }[] = [];

    // Traverse and separate wheels from the body
    rawObject.traverse(child => {
      if ((child as THREE.Mesh).isMesh) {
        const mesh = child as THREE.Mesh;
        ...
        const isWheel = name.includes('wheel') || 
                        ...
                        name.includes('lop') || 
                        name.includes('lốp') || 
                        name.includes('banhxe') || 
                        name.includes('banh_xe');

        if (isWheel) {
          const pos = new THREE.Vector3().setFromMatrixPosition(mesh.matrixWorld);
          const isFront = pos.z >= modelCenter.z;
          const isLeft = pos.x <= modelCenter.x;

          if (isFront && isLeft) {
            flGeos.push({ mesh, pos });
          } else if (isFront && !isLeft) {
            frGeos.push({ mesh, pos });
          } else if (!isFront && isLeft) {
            rlGeos.push({ mesh, pos });
          } else {
            rrGeos.push({ mesh, pos });
          }
        } else {
    ...
    // Helper to calculate center of wheel parts
    const calcGroupCenter = (items: { mesh: THREE.Mesh; pos: THREE.Vector3 }[]) => {
    ...
    // Robust, optimized helper to merge a list of meshes by material signature
    const mergeMeshes = (
      meshes: THREE.Mesh[],
      ...
      groups.forEach((groupVal, sig) => {
        ...
        try {
          // If there's only one geometry, no need to merge, just use it
    ...
    // Helper to create optimized wheel groups
    const createWheelGroup = (name: string, items: { mesh: THREE.Mesh; pos: THREE.Vector3 }[]) => {
      const wheelGroup = new THREE.Group();
      wheelGroup.name = name;

      if (items.length === 0) return wheelGroup;

      const center = calcGroupCenter(items);
      wheelGroup.position.copy(center);

      const meshesOnly = items.map(i => i.mesh);
      const wheelRelativeMatrix = (mesh: THREE.Mesh) => {
      ...
      const mergedWheelMeshes = mergeMeshes(meshesOnly, wheelRelativeMatrix);
      mergedWheelMeshes.forEach(m => wheelGroup.add(m));

      return wheelGroup;
    };

    const hasWheels = flGeos.length > 0 || frGeos.length > 0 || rlGeos.length > 0 || rrGeos.length > 0;

    if (hasWheels) {
      const wheelFL = createWheelGroup('wheel_fl', flGeos);
      const wheelFR = createWheelGroup('wheel_fr', frGeos);
      const wheelRL = createWheelGroup('wheel_rl', rlGeos);
      const wheelRR = createWheelGroup('wheel_rr', rrGeos);
      contentGroup.add(wheelFL, wheelFR, wheelRL, wheelRR);
    } else {
      bodyMeshes.push(...flGeos.map(i => i.mesh));
      bodyMeshes.push(...frGeos.map(i => i.mesh));
      bodyMeshes.push(...rlGeos.map(i => i.mesh));
      bodyMeshes.push(...rrGeos.map(i => i.mesh));
    }
  ...
  public attachCarVisual(
    ...
    const swapInModel = (cached: THREE.Group) => {
      ...
      // Find real wheels inside clone
      const wFL = clone.getObjectByName('wheel_fl');
      const wFR = clone.getObjectByName('wheel_fr');
      const wRL = clone.getObjectByName('wheel_rl');
      const wRR = clone.getObjectByName('wheel_rr');

      if (wFL && wFR && wRL && wRR) {
        carObj.wheels.length = 0; // Clear the old array in-place
        carObj.wheels.push(wFL, wFR, wRL, wRR); // Push the new wheels in-place so caller references get updated correctly
      }
    };
