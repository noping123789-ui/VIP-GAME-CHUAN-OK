...
export function generatePointsForLayout(layout: string, seed: number = 42): THREE.Vector3[] {
  ...
  const startPerturbY = (rand() - 0.5) * 15;
  ...
  points.forEach((p, idx) => {
    if (idx === 0 || idx === points.length - 1) {
      ...
      p.x += (rand() - 0.5) * 180;
      p.y += (rand() - 0.5) * 40; // Giới hạn dốc đứng tránh đường nhấp nhô gãy khúc
      p.z += (rand() - 0.5) * 180;
    }
    p.y = Math.max(2.5, p.y); // Đảm bảo mặt đường luôn nằm cao hơn mặt đất phẳng để tránh đứt đoạn
  });
