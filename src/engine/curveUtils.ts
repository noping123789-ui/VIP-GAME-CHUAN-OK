...
export function safeGetPointAt(curve: THREE.Curve<THREE.Vector3>, u: number): THREE.Vector3 {
  ...
  try {
    ...
    // Đảm bảo tọa độ y của đường luôn tối thiểu là 1.5 để không bị lún xuống dưới mặt đất phẳng (y = -0.5), giải quyết triệt để lỗi đường đứt đoạn
    return new THREE.Vector3(pt.x, Math.max(1.5, pt.y), pt.z);
  } catch {
    return new THREE.Vector3(0, 0, 0);
  }
}
