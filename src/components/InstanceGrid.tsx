...
export const InstanceGrid: React.FC<InstanceGridProps> = ({
  ...
  const getCameraLabelVN = (mode: CameraMode) => {
    switch (mode) {
      ...
      case CameraMode.SPECTATOR_TRACKSIDE: return 'Spectator Ven Đường';
      case CameraMode.HELIPAD_ZOOM: return 'Helipad Zoom Xa';
      case CameraMode.SATELLITE_ORBIT: return 'Vệ Tinh Orbit';
      ...
      // 12 góc cinematic cũ + mới
      case CameraMode.BEHIND: return '1. Phía Sau Xe';
      case CameraMode.HOOD: return '2. Mui Xe/Capo';
      ...
      case CameraMode.COCKPIT_FIRST_PERSON: return '11. Góc Lái Thứ 1';
      case CameraMode.BUMPER_FIRST_PERSON: return '12. Cản Trước Siêu Tốc';
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Driver List Overlay - Professional Broadcast Starting Grid from Second 3 to Second 6 (Compact Left Sidebar Overlay, never blocking screen) */}
                {instance.chunkTimeElapsed >= 3.0 && instance.chunkTimeElapsed <= 6.0 && (
                  <div className="absolute top-2.5 left-2.5 w-[210px] bottom-2.5 bg-slate-950/90 backdrop-blur-md z-40 p-2.5 flex flex-col rounded-lg border border-slate-850 shadow-2xl animate-fade-in font-sans pointer-events-auto">
                    <div className="mb-2 border-b border-cyan-500/30 pb-1.5 shrink-0">
                      <div className="text-[8px] font-mono tracking-[0.15em] text-cyan-400 font-bold uppercase mb-0.5 flex items-center gap-1">
                      ...
                      <p className="text-[7px] text-slate-400 font-mono">
                        LUỒNG #{instance.id.toString().padStart(2, '0')} • SEED {instance.seedData.seed}
                      </p>
                    </div>

                    <div className="flex flex-col gap-1 overflow-y-auto flex-grow pr-1 custom-scrollbar">
                      {[...instance.cars]
                        .sort((a, b) => a.id.localeCompare(b.id)) // Stable sort by ID so the list never jitters or jumps during starting sequence
                        .map((car, idx) => {
                          ...
                          return (
                            <div
                              key={car.id}
                              className="flex items-center justify-between bg-slate-900/90 border border-slate-800/50 rounded p-1 hover:border-cyan-500/20 transition-all shrink-0"
                            >
                              <div className="flex items-center gap-1.5 truncate">
                                {/* Grid Position */}
                                <div className="w-4 h-4 rounded bg-cyan-950 border border-cyan-500/10 flex items-center justify-center font-mono font-black text-cyan-400 text-[8.5px] shrink-0">
                                  {idx + 1}
                                </div>
                                ...
                                <div className="flex flex-col truncate leading-tight">
                                  <span className="font-bold text-white uppercase text-[8px] tracking-wide truncate">
                                    {car.driverName}
                                  </span>
                                  <span className="text-[6.5px] text-slate-400 font-mono truncate">
                                    {car.name}
                                  </span>
                                </div>
                              </div>
                              ...
                              {/* Category Badge */}
                              <div className="text-[6.5px] font-mono font-bold text-cyan-400/90 bg-cyan-950/40 px-1 py-0.2 rounded border border-cyan-500/5 shrink-0">
                                {car.type === 'hypercar' ? 'HYPER' : 'F1'}
                              </div>
                            </div>
                          );
                        })}
                    ...
                    <div className="mt-2 text-center border-t border-slate-800/80 pt-1.5 shrink-0">
                      <div className="text-[7px] text-slate-500 font-mono uppercase tracking-[0.05em]">
                        Cuộc Đua Sắp Bắt Đầu • Khởi Động
                      </div>
                    </div>
                  </div>
                )}
